...
- [ ] M_max/P_max=4/3 对现有 9 条经验的 formalSpec 逐 op 核对:file_read{path}/file_write{path,content}✓ / shell_exec{command,args}✓ / glob_match{pattern,path,cwd}→ in0,in1,in2 ✓ / grep_search{pattern,path}✓ / string_replace{text,find,replace}=in0,in1,in2 + literal sidecar? replace_all=true 是 OpStep.inputs 里的 kind:'literal' → expandRefs 走 argtmp pool(k=0 轮转),**不占 in-slot**(input slots 只给 inputs map key;字面量参数走 pool)—— ⚠️ **需确认**:formalSpec.input 是否允许 value 由 expandRefs 动态决定而不必在 FormalParam.register 静态声明?(当前 FormalParam.register='$r0'/'$r1…' 是编译期硬编码字符串,P1 改造后该字段应改为 `slotIndex:number`,compiler expandRefs 根据 ParamRef.kind 决定实际 Address.name —— literal→pool[inputSlotIndexOf(name)]、register→parent outK;这个"FormalParam.register 从 string 改 number slot index"的 schema 变更是本设计最大潜在返工点,demo/测试若有直接读 .register 字段的断言需同步迁移)
