...
**U_max 重新核算(C1)**：Global{$err,$path}=2 + literal pool(8)+ judge/cond pool(per-experience max 10，但同 experience 内串行、不同 frame scope 隔离 → **活跃数=max(单experience最大并发条件判断)≈J_max≤5 → 2×5=10**) + in-slots(M_max=4) × max_concurrent_scopes(栈深度 N_depth,递归经验上限1000!⚠️需另算见下)+ out-slots(P_max=3)。
- ⚠️ **关键**:in/out slots 是 per-frame-scope 的($S<scope>.*)，所以总量随**同时存活的嵌套帧数**(=递归深度)线性增长——这是 CALL/RET 语义下的固有特性(x86 每层 stack frame 也有自己的 RBP shadow;CPU 也靠"通用寄存器物理上只有一份、frame 切换时 save/restore 到栈"实现类似效果)。MVP 缓解策略:**P1 起引入 `maxActiveScopes` 软限 = min(maxRecursionDepth, 8)**,超出即 RecursionDepthError(现有 A11 机制加一个并列检查即可);真实业务递归经验(如 retry-until-success)通常 ≤5 轮,8 足够且让 U_max 有界 ≈ 2+8+10+ (4+3)×8 ≈ 76 —— 仍远小于今天实际无界的 argtmp 累计。若嫌 76 太大,P2/P3 可把 literal/judge pool 从 global-scope 改为 **per-scope but bounded by pool size**,进一步压低峰值。**本设计先给可行上界,U_max 最终值由 P4 实测固化(doc N4/R6b const table 是进一步优化方向,非阻塞项)**。
...
| **P4** (可选收尾)R6b const table / U_MAX实测固化到doc / legacy mode删除 & alias清理 | 视P1–P3剩余债务决定;U_max从"76估算"改为"P4跑全部demo+tier-c e2e取峰值并写入[需求文档](./19-register-file-core.md)#C1";errors.ts ERROR_REGISTER='$r_err'别名正式删(改用GLOBAL_ERR直接export,response.ts/bubbleError同步改import) | errors.ts、main-loop.bubbleError(ERROR_REGISTER import源切换)、pipeline/response(残留旧名扫描) | internalStore.size()峰值断言用P4实测定值替换§三的估算值;全库rg '\$r_input_|\$argtmp_|\$r_cond_\|$r_judge_\|PRIMARY_OUTPUT'清零(deprecated alias可保留一迭代给外部demo脚本迁移期,demos/*.ts若有硬编码引用需人工review——目前grep显示demos只import public API,$S*是compiler内部产物不泄漏,预期无破坏性变更) |
